# KepamMeter Questions

16 questions, fixed order. Each answer carries a weight; the last option on every question is the heaviest (20).

**Score** = points earned ÷ maximum possible × 100, rounded. Skipped questions are excluded from both numbers.
Maximum possible with every question answered: 320 points = 100%.

**Angry mode** (red lights, shaking, angry model, spilled matcha) starts above **40%**.

## Diagnosis bands

| Score | Diagnosis |
|---|---|
| 0 to 30% | 😇 Pure Soul |
| 31 to 70% | 😌 Average Netizen |
| 71 to 100% | 🔥 Certified Kepamist |

## Questions

### 1. Do you like to cut the queue?

| Answer | Points |
|---|---|
| Never. | 0 |
| Rarely. Only if I'm really late. | 6 |
| Sometimes. Nobody notices. | 13 |
| Often. And I scold people back if they scold me. | 20 |

### 2. You're stressed. What do you do?

| Answer | Points |
|---|---|
| Drink mineral water. | 0 |
| Buy a matcha. | 7 |
| Buy ayam gepuk. | 13 |
| Buy matcha AND gepuk. Terpaling stressed. | 20 |

### 3. Do you use the word “terpaling”?

| Answer | Points |
|---|---|
| What is that? | 0 |
| Ironically, sometimes. | 6 |
| Daily. Terpaling daily. | 14 |
| It's my whole personality. | 20 |

### 4. How often do you say “guys, you know what, I said what I said, periodt, literally, hello??”

| Answer | Points |
|---|---|
| Never. | 0 |
| Once in a while, ironically. | 6 |
| Daily. Literally. | 13 |
| Every sentence. Hello?? Periodt. | 20 |

### 5. Do you use CAPS LOCK when arguing online?

| Answer | Points |
|---|---|
| Never. I use full stops and walk away. | 0 |
| Only when they are WRONG. | 7 |
| MY KEYBOARD IS STUCK LIKE THIS. | 14 |
| I TYPE LIKE THIS TO MY OWN GRANDMA. | 20 |

### 6. Do you hate men?

| Answer | Points |
|---|---|
| No. I love everyone. | 0 |
| Only the ones in the comments. | 6 |
| Only online. In real life I'm very polite. | 13 |
| Yes, and I post about it daily. Periodt. | 20 |

### 7. How fast do you reply to viral drama?

| Answer | Points |
|---|---|
| I don't follow drama. | 0 |
| After it hits the news. | 5 |
| Within the hour, with screenshots. | 13 |
| Reply? I AM the drama. | 20 |

### 8. Someone gets cancelled for a tweet from 2016. You…

| Answer | Points |
|---|---|
| Don't care. It's 2016. | 0 |
| Read the thread quietly. | 6 |
| Add a “yikes” comment. | 13 |
| Dig up their 2014 tweets too. | 20 |

### 9. Someone posts a photo of their nasi lemak. You…

| Answer | Points |
|---|---|
| Double tap and move on. | 0 |
| Comment “where is this?” | 5 |
| Comment “RM12? terpaling mahal.” | 13 |
| Rate it 4/10 and tag a food influencer. | 20 |

### 10. The comment section is already 500 copies of the same insult. You…

| Answer | Points |
|---|---|
| Close the app. | 0 |
| Read a few for entertainment. | 6 |
| Add a more original insult. | 13 |
| Copy-paste the popular one so I don't miss out. | 20 |

### 11. Your friend's post gets 10k likes. Your first thought is…

| Answer | Points |
|---|---|
| Happy for them! | 0 |
| Nice, but mine was funnier. | 7 |
| Screenshot to the group chat with 🙄 | 14 |
| Post a story: “reminder that likes don't matter” | 20 |

### 12. Your group chat spots a stranger's cringe post. First move?

| Answer | Points |
|---|---|
| Leave it alone. | 0 |
| Laugh privately. | 5 |
| Forward the link to two other group chats. | 13 |
| Everyone comments at once. Macam biasa. | 20 |

### 13. Someone apologises online. You…

| Answer | Points |
|---|---|
| Accept it and move on. | 0 |
| Wait and see if they mean it. | 6 |
| “Apology not accepted” + a GIF. | 13 |
| Screenshot it into the receipts folder. | 20 |

### 14. Your go-to caption style?

| Answer | Points |
|---|---|
| No caption. The photo speaks. | 0 |
| One emoji. | 5 |
| A paragraph about “the journey”. | 13 |
| “Unpopular opinion but…” followed by a very popular opinion. | 20 |

### 15. The report button. How often?

| Answer | Points |
|---|---|
| Only for real harm. | 0 |
| Occasionally. | 5 |
| Whenever I disagree. | 13 |
| Mass-reporting with friends is a hobby. | 20 |

### 16. People wish you happy birthday on your wall. You…

| Answer | Points |
|---|---|
| Say thanks once, to everyone. | 0 |
| Like every comment. | 5 |
| Reply “thank you sayang” to every single one. | 13 |
| Repost all of them into a 47-slide story. | 20 |
